...
class BaseHTTPMercadoPago:
        ...
        def organiza_get_request(self, url: str) -> ResultadoPagina:
            ...
            """
            response = self.adapt_pedidos.get_endpoints_mercadopago(self.access_token, url)

            if response.status_code == 200:
                data = response.json()
                logger.info('Mercado pago organiza_get_request | Retorno da endpoint %s', response.status_code)
